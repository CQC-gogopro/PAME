...
from .MoE_mamba import MoeMamba
from .order_h import get_h_order, get_h_order_4, get_h_order_8
from .order_s import get_s_order_4
from .order_z import get_z_order_4
import numpy as np
...
class PAME(nn.Module):
    def __init__(self, p, select_list, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12,
        ...
        self.order_map_list_stage = {}
        
        for s in range(self.num_layers):
            # self.order_map_list_stage[s] = get_h_order_4(H=normalized_shape[s][1],W=normalized_shape[s][2])
            # self.order_map_list_stage[s] = get_h_order_4(H=p.H,W=p.W) + get_s_order_4(H=p.H,W=p.W)
            if self.p.Scan == 'z':
                self.order_map_list_stage[s] = get_z_order_4(H=p.H,W=p.W)
            elif self.p.Scan == 's':
                self.order_map_list_stage[s] = get_s_order_4(H=p.H,W=p.W)
            else:
                self.order_map_list_stage[s] = get_h_order_4(H=p.H,W=p.W)
        get_s_order_4
        
        
        if not self.p.BASELINE:
            self.stage_MTLoRA_mamba = nn.ModuleList(MoeMamba(p,
                                                            d_model=final_embed_dim,
                                                            d_state=p.dstate,
                                                            ssm_ratio=2,
                                                            tasks=self.tasks,
                                                            order_map_list=self.order_map_list_stage[s],
                                                            fusion_depth = 4,
                                                            tfe_depth = 4,
                                                            mlp_ratio = 0,
                                                            rank_list = p.rank_list,
                                                            Nmoe = 15,
                                                            topk = p.topk if 'topk' in p.keys() else 9,) if s>=p.overstage else None
                                                    for s in range(self.num_layers))
        ...
        for task in p.TASKS.NAMES:
            if not self.p.BASELINE:
                self.task_mask[task] = nn.Sequential(
                                                    nn.Conv2d(final_embed_dim * (4-p.overstage), final_embed_dim, kernel_size=1),
                                                    BatchNorm2d(final_embed_dim), nn.GELU(),  
                                                    nn.Conv2d(final_embed_dim, self.num_layers, kernel_size=3, padding=1))
            else:
                self.task_mask[task] = nn.Sequential(
                                                    nn.Conv2d(p.backbone_dim * (4-p.overstage), final_embed_dim, kernel_size=1),
                                                    BatchNorm2d(final_embed_dim), nn.GELU(),  
                                                    nn.Conv2d(final_embed_dim, self.num_layers, kernel_size=3, padding=1))
    ...
    def forward(self, x, p):
        ...
        last_feat = {task:0 for task in self.tasks}
        out_mask = {task: 0 for task in self.tasks}
        info = {} # pass information through the pipeline

        for idx, blk in enumerate(self.blocks):
            x, attn_weight = blk(x)  # x: b l d 
            if idx + 1 in self.select_list: 
                il = np.sum(idx >= (np.array(self.select_list) - 1)) - 1 # [0,1,2]
                if il>=self.p.overstage:
                    # extract task-specific feature at this layer
                    if not self.p.BASELINE:
                        _task_fea = self.stage_MTLoRA_mamba[il](x ,self.p.H, self.p.W) 
                        for task in self.tasks:
                            out_feat[task].append(_task_fea[task])
                    else:
                        for task in self.tasks:
                            if len(x.shape) == 3:
                                out_feat[task].append(rearrange(x, "b (h w) d -> b d h w", h=self.p.H, w=self.p.W))
                                continue
                            out_feat[task].append(x)
        # extract task-specific feature at the last layer
        il=self.num_layers-1
        x = self.norm(x)

        if not p['__train__']:
            if p['backbone_stage3_dirs']:
                info = {'meta':p['meta'],
                        'save_path':p['backbone_stage3_dirs']}
                info['__train__'] = p['__train__']
                info['im_size'] = (p.H,p.W)
                img = rearrange(x, "b (h w) d -> b d h w", h=self.p.H, w=self.p.W)
                pca_vis(img, info, zoom_rate=16)
        ...
        for task in self.tasks:
            ...
            for il in range(self.num_layers-self.p.overstage):
                last_feat[task] = last_feat[task] + mask_now[:, il].unsqueeze(1) * out_feat[task][il]         #TODO check
        # 插值输出
...
def _create_PAME(variant, pretrained=False, default_cfg=None, **kwargs):
    default_cfg = default_cfg or default_cfgs[variant]
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')

    model = build_model_with_cfg(
        PAME, variant, pretrained,
...
def PAME_vit_large_patch16_384(pretrained=False, **kwargs):
    ...
    model = _create_PAME('vit_large_patch16_384', pretrained=pretrained, **model_kwargs)
    return model
...
class ConvHead(nn.Module):
    ...
    def forward(self, x, info):
        ...
        """
        x1 = self.mt_proj(x)    # torch.Size([6, 1024, 128, 128])
        
        if not info['__train__']:
            if info['head_tsne_save_dirs'] or info['head_pca_save_dirs']:
                # 对每张图片单独进行 PCA
                for i, img in enumerate(x):
                    img_size = info['meta']['img_size'][i]
                    h,w = img_size[0], img_size[1]
                    if h<w:
                        crop = int(((info['im_size'][0]*16-h)/4)/2)
                        img = img[:,crop:-crop,:]
                    if h>w:
                        crop = int(((info['im_size'][1]*16-w)/4)/2)
                        img = img[:,:,crop:-crop]
                    img_reshaped = rearrange(img,"C H W->(H W) C").cpu().numpy()
                    if info['head_pca_save_dirs']:
                        # 应用 PCA
                        pca = PCA(n_components=1)
                        img_pca = pca.fit_transform(img_reshaped)
                        
                        # 重塑结果回到 (H, W)
                        img_reduced = img_pca.reshape(img.shape[1], img.shape[2])
                        
                        # 插值回原始分辨率
                        img_reduced = zoom(img_reduced, (4,4), order=3)
                        
                        # 归一化处理，转换为0到1的浮点数
                        normalized_img = (img_reduced - img_reduced.min()) / (img_reduced.max() - img_reduced.min())
                        # 应用热力图色彩映射
                        colormap = plt.get_cmap('jet')  # 可以选择 'viridis', 'plasma', 'inferno', 'magma', 'cividis'
                        heatmap = colormap(normalized_img)
                        
                        # 将热力图转换为0到255的整数形式的 RGB 图像
                        heatmap_image = (heatmap[:, :, :3] * 255).astype(np.uint8)
                        
                        # 创建 PIL 图像并保存
                        image = Image.fromarray(heatmap_image)
                        # image.save(os.path.join(info['head_pca_save_dirs'][info['task']],str(info['meta']['img_name'][i])+'.png'))
                        image.save(os.path.join(info['head_pca_save_dirs'][info['task']],str(info['meta']['img_name'][i])+'_30000.png'))
                        
                        if info['head_vis_pca_reverse']:     # 特征取反再来一遍
                            reverse_img =  1 - normalized_img
                            # 应用热力图色彩映射
                            colormap = plt.get_cmap('jet')  # 可以选择 'viridis', 'plasma', 'inferno', 'magma', 'cividis'
                            heatmap = colormap(reverse_img)
                            ...
                            # 创建 PIL 图像并保存
                            image = Image.fromarray(heatmap_image)
                            # image.save(os.path.join(info['head_pca_save_dirs'][info['task']],str(info['meta']['img_name'][i])+'_r'+'.png'))
                            image.save(os.path.join(info['head_pca_save_dirs'][info['task']],str(info['meta']['img_name'][i])+'_30000_r'+'.png'))
        
        return self.linear_pred(x1)
...
def pca_vis(x, info, zoom_rate=4):
    if not info['__train__']:
        # 对每张图片单独进行 PCA
        for i, img in enumerate(x):
            img_size = info['meta']['img_size'][i]
            h,w = img_size[0], img_size[1]
            if h<w:
                crop = int(((info['im_size'][0]*16-h)/zoom_rate)/2)
                img = img[:,crop:-(crop-1),:]
            if h>w:
                crop = int(((info['im_size'][1]*16-w)/zoom_rate)/2)
                img = img[:,:,crop:-(crop-1)]
            img_reshaped = rearrange(img,"C H W->(H W) C").cpu().numpy()
            if info['save_path']:
                # 应用 PCA
                ...
                # 插值回原始分辨率
                img_reduced = zoom(img_reduced, (zoom_rate,zoom_rate), order=3)
                ...
                # image.save(os.path.join(info['head_pca_save_dirs'][info['task']],str(info['meta']['img_name'][i])+'.png'))
                path = info['save_path']
                image.save(os.path.join(path, str(info['meta']['img_name'][i])+'.png'))
                ...
                image = Image.fromarray(heatmap_image)
                # image.save(os.path.join(info['head_pca_save_dirs'][info['task']],str(info['meta']['img_name'][i])+'_r'+'.png'))
                image.save(os.path.join(path, str(info['meta']['img_name'][i])+'_r.png'))