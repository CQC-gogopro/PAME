...
from evaluation.depth_vision import depth_save
from evaluation.semseg_vision import semseg_save
...
def save_model_pred_for_one_task(p, batch_idx, sample, output, save_dirs, task=None, epoch=None):
    ...
    for jj in range(int(inputs.size()[0])):
        ...
        if task == 'depth':
            ...
            depth_save(result, os.path.join(save_dirs[task], fname + '.png'))
        elif task == 'semseg':
            semseg_save((result+1).astype(np.uint8), os.path.join(save_dirs[task], fname + '.png'))
        elif task == 'edge':
